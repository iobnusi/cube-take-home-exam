...
export async function fetchTopProducts(
  ...
  const res = await fetch(`${BASE_URL}/top/products${qs}`, {
    cache: 'no-store',
  });
  if (!res.ok) throw new Error('Failed to fetch /top/products');
  return res.json() as Promise<TopProductsResponse>;
}

export async function fetchTopShops(
  filters: FilterParams,
  rankBy: TopShopsRankBy = 'nmv',
  ...
  const res = await fetch(`${BASE_URL}/top/shops${qs}`, {
    cache: 'no-store',
  });
  if (!res.ok) throw new Error('Failed to fetch /top/shops');
  return res.json() as Promise<TopShopsResponse>;
}
