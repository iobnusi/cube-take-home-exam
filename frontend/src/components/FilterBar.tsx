...
export default function FilterBar() {
  ...
  useEffect(() => {
    ...
    fetchL1Categories()
      .then((opts) => { if (!controller.signal.aborted) setL1Options(opts); })
      .catch(() => { if (!controller.signal.aborted) setL1Options([]); });
  ...
  useEffect(() => {
    ...
    fetchL2Categories(filters.l1_category)
      .then((opts) => { if (!controller.signal.aborted) setL2Options(opts); })
      .catch(() => { if (!controller.signal.aborted) setL2Options([]); });
    return () => controller.abort();
  }, [filters.l1_category]);
  ...
  useEffect(() => {
    ...
    fetchL3Categories(filters.l1_category, filters.l2_category)
      .then((opts) => { if (!controller.signal.aborted) setL3Options(opts); })
      .catch(() => { if (!controller.signal.aborted) setL3Options([]); });
    return () => controller.abort();
  ...
  useEffect(() => {
    ...
    fetchL4Categories(
      ...
      .then((opts) => { if (!controller.signal.aborted) setL4Options(opts); })
      .catch(() => { if (!controller.signal.aborted) setL4Options([]); });
    return () => controller.abort();
  ...
  return (
    <div className="px-6 py-3 bg-white border-b border-slate-200 space-y-2">
      {/* Row 1: platform, region, origin, mall status, date range */}
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          onChange={(e) => setFilter("is_mall", (e.target.value as IsMall) || undefined)}
          className={SELECT_CLASS}
          style={CHEVRON_STYLE}
        >
      ...
      {/* Row 2: cascading category dropdowns */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs text-slate-400 w-16 text-right mr-1">Category</span>
        ...
        <select
          ...
          onChange={(e) => setFilter("l4_category", e.target.value || undefined)}
